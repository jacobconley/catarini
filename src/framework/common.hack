...
abstract class BaseFramework
 {
	...
	public function setRoute(string $Route) : void { 
		if($this->Route != NULL) throw new FrameworkException('Route has already been set'); 
		$this->Route = $Route;
	}
	/**
	...
	public function getRoute() : string {
		if($this->Route == NULL) throw new FrameworkException('No route has been set');
		return $this->Route; 
	}
	...
	/**
	 * Routing function.  If the request uri matches and the proper method is used, $function will be executed. 
	 * @param $methods [arrayable] HTTP METHOD to match ('GET' or 'POST')
	 * @param $route [arrayable] List of routes to match REQUEST_URI to.  The first root is the "canonical" route and will be preferred to alternatives
	 * @param $function Callback function, taking this framework instance as a parameter.
	 * @param $final If TRUE, exit with the appropriate HTTP status after evaluation.  If FALSE, continue as normal
	 * @return true if matched, otherwise false
	 */
 	public function Route(mixed $methods, mixed $route, (function (this) : void) $function, bool $final = TRUE) : void 
	{
		if(! \is_array($methods)) $methods = [$methods];
		if(! \is_array($route)) $route = [$route]; 

		$request_route = \explode('?', $this->partial->getRequestURI(), 2)[0];
		if(\in_array($request_route, $route)) {

			if(\in_array($this->partial->getRequestMethod(), $methods)) { 
				$this->setRoute($route[0] as string); // Prefer first route  
				$function($this);
				
				if($final) \exit(0);
			}
			else if($final) { 
				\http_response_code(405); // Bad request method
				\exit(0);
			}
	...
	public function LoadEndpointClass(string $endpoint) : void { 
		$classname = "endpoint__$endpoint";
		$class = new \ReflectionClass($classname); 
		$attrs = $class->getAttributes(); 
		$inst  = $class->newInstance();
		$request = \explode('?', $this->partial->getRequestURI(), 2)[0];

		$validMethod = TRUE;
		if(\array_key_exists('HTTP_METHOD', $attrs) && !\in_array($this->partial->getRequestMethod(), $attrs['HTTP_METHOD'])) $validMethod = FALSE; 

		if(\array_key_exists('HTTP_STATIC', $attrs) && \in_array($request, $attrs['HTTP_STATIC'])) {
			if(! $validMethod) { 
				\http_response_code(405); // Bad request method
				\exit(0); 
			}

			$class->getMethod('RENDER')->invoke($inst, $this);
			\exit(0); 
		}

		if(\array_key_exists('HTTP_DYNAMIC', $attrs)) { 
			if(! $validMethod) { 
				\http_response_code(405); // Bad request method
				\exit(0); 
			}

			foreach($attrs['HTTP_DYNAMIC'] as $regex) { 
				/* HH_IGNORE_ERROR[4110] Telling lies */
				$match = Regex\first_match($request, $regex);
				if($match) { 
					$class->getMethod('RENDER')->invoke($inst, $this, $match); 
				}
			}
		}
	}

	public function LoadEndpointDirectory(string $dir) : void { 
		$handle =  \opendir($dir);
		if($handle) {
			for($entry = \readdir($handle); $entry !== FALSE; $entry = \readdir($handle)) {

				$match = Regex\first_match($entry, re"#([a-zA-Z_]+)\.hack$#");
				if($match) { 
					$this->LoadEndpointClass($match[1]); 
				}
			} 
			\closedir($handle);
			return;
		}
		else throw new FrameworkException("Could not open endpoints directory $dir");
	}
